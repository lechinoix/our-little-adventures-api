...
const generateThumbnail = async file => {
  const fileBuffer = await convertToProcessableFormat(file.buffer);
...
const optimize = async buffer => {
  ...
  const fileBuffer = await convertToProcessableFormat(buffer);
...
const convertToProcessableFormat = async buffer => {
  const { format } = await getMetadatas(buffer);
  return formatsToConvert.includes(format)
    ? sharp(buffer)
      .jpeg({ quality: 100 })
      .toBuffer()
    : buffer;
